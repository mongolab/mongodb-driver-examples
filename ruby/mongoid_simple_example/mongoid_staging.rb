...
Mongoid.logger.formatter = Logger::Formatter.new
Mongoid.logger.level = Logger::DEBUG
Mongoid.load!('mongoid2.yml', :production)
### Define Schema
...
#Song.where(:weeksAtOne.gte => 10).sort(decade: 1).no_timeout.each do |doc| puts "In the #{ doc['decade'] }," +
# " #{ doc['song'] } by #{ doc['artist'] }" +
# #" topped the charts for #{ doc['weeksAtOne'] }" +
# " straight weeks." 
#end

#songs.all.no_timeout.each{ |doc| puts "In the #{ doc['decade'] }," +
#                        " #{ doc['song'] } by #{ doc['artist'] }" +
#                        " topped the charts for #{ doc['weeksAtOne'] }" +
#                        " straight weeks." }
